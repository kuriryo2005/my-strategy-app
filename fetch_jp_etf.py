...
def fetch_jp_etf(
    ...
    """TOPIX-17 業種ETF の OHLCV を yfinance で取得する（一括ダウンロード）.
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            logger.info("Downloading JP ETFs batch (attempt %d/%d)...", attempt, _MAX_RETRIES)
            df = yf.download(
                tickers,
                start=start,
                end=end,
            ...
            if df is not None and not df.empty:
                ...
                if len(missing) == 0:
                    logger.info("Successfully downloaded all JP ETFs.")
                    break
                else:
                    logger.warning("Missing JP tickers: %s. Retrying...", missing)
            else:
                logger.warning("Downloaded empty JP DataFrame. Retrying...")
    ...
    if df is None or df.empty:
        raise RuntimeError("JP ETFのデータ取得に失敗しました（空のデータ）")
